...
map_mods <- function(myFactors){ #this calculates the bonuses for each feat/style/force
	...
	if(all(myFactors$styles=="Dueling 4")){
        myFactors$modifiers1 <- myFactors$modifiers1+3
    }
    if(all(myFactors$styles=="Dueling 5")){
        myFactors$modifiers1 <- myFactors$modifiers1+3
    }
    if(all(myFactors$styles=="Dueling 6")){
        myFactors$modifiers1 <- myFactors$modifiers1+3
    }
	...
	if(all(myFactors$styles=="Two-Weapon Fighting 4")){
        myFactors$n2 <- 1
        myFactors$modifiers1 <- myFactors$modifiers1-1
        myFactors$modifiers2 <- myFactors$modifiers2-2
    }
	if(all(myFactors$styles=="Two-Weapon Fighting 5")){
        myFactors$n2 <- 1
        myFactors$modifiers2 <- myFactors$modifiers2-2
    }
	if(all(myFactors$styles=="Two-Weapon Fighting 6")){
        myFactors$n2 <- 1
    ...
    if(all(myFactors$forms=="Shii-Cho")){
        myFactors$modifiers1 <- myFactors$modifiers1+1
        myFactors$modifiers2 <- myFactors$modifiers2+1
    }
    if(all(myFactors$forms=="Makashi")){
        myFactors$power_attack <- myFactors$power_attack + 3
        myFactors$modifiers1 <- myFactors$modifiers1+3
        myFactors$modifiers2 <- myFactors$modifiers2+3
    }
    if(all(myFactors$forms=="Soresu")){
        myFactors$threat_bonus <- myFactors$threat_bonus - 0.05
    }
    if(all(myFactors$forms=="Ataru")){
        myFactors$threat_bonus <- myFactors$threat_bonus + 0.05
    }
    if(all(myFactors$forms=="Shien")){
        myFactors$modifiers1 <- myFactors$modifiers1 + 2
        myFactors$modifiers2 <- myFactors$modifiers2 + 2
        myFactors$crit_mult <- myFactors$crit_mult + 1
    }
    if(all(myFactors$forms=="Niman")){
        myFactors$modifiers1 <- myFactors$modifiers1 + 1
        myFactors$modifiers2 <- myFactors$modifiers2 + 1
    }
    if(all(myFactors$forms=="Juyo")){
        myFactors$n1 <- myFactors$n1 + 1
        myFactors$crit_modifier <- myFactors$crit_modifier + 4
    }
    return(myFactors) 
}
...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("KotOR 2 Combat Feat Comparisons"),
            ...
            fluidRow(
                column(width=4, 
                       ...
                       radioButtons("feats", "Combat Feat Level: 
                        ...
                        radioButtons(
                            "forms", "Lightsaber Forms", 
                            choiceNames = form_choices,
                            choiceValues = form_choices
                        ),
                    ),
                    column(width=4, 
                            ...
                            radioButtons(
                                ...
                                choiceNames = c("None", "Level 1", "Level 2", "Level 3", "Superior 1", "Superior 2", "Superior 3"),
                                choiceValues = c("None", "1", "2", "3", "4", "5", "6")
                            )
                    )
                )
            ),
...
server <- function(input, output) {
    ...
    model_factors <- 
        ...
        group_by(styles, feats, force, forms) %>% 
        group_split() %>% 
        map(map_mods) %>% 
        bind_rows()
    ...
    observe({ 
            ...
            mutate(
                ...
                total_damage =
                    n1 * (DMG_main + power_attack) * (
                        1 * pmax(pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers1 - balance), 0.95), 0.05) - threat_main * (threat_mult + keen_main) - threat_bonus, 0) + # Non-threat hit
                        pmin(threat_main * (threat_mult + keen_main) + threat_bonus, pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers1 - balance), 0.95), 0.05)) * # Check threat
                        (1 + crit_mult * pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers1 - balance - crit_modifier), 1), 0))  # Crit roll has no automatic success / failure
                    ) +
                    n2 * (DMG_offhand + power_attack) * (
                        1 * pmax(pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers2), 0.95), 0.05) - threat_offhand * (threat_mult + keen_offhand) - threat_bonus, 0) + # Non-threat hit
                        pmin(threat_offhand * (threat_mult + keen_offhand) + threat_bonus, pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers2), 0.95), 0.05)) * # Check threat
                        (1 + crit_mult * pmax(pmin(1.05 - 0.05 * (DEF_ATK - modifiers2 - crit_modifier), 1), 0))  # Crit roll has no automatic success / failure
                    )
            ) %>% 
        ...
        output$plot <- renderPlotly({
            ...
            model_factors %>% 
                ...
                filter(grepl(forms_choice, forms)) %>% 
